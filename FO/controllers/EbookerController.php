...
Class EbookerController Extends ContribController
{
    ...
    public function sendStencilsAction()
    {
		...
		//$replace = array('oe', 'Oe', 'Ae', 'ae');
                ...
                $stencils_text=$stencils_text_file=implode( "###$$$###",preg_replace($patterns,$replace,$missionParams['stencil_text']) );
...
//		echo "<pre>";print_r($missionParams); exit;
    ...
    public function sendCorrectorStencilsAction()
    {
		...
		/*getting and combining stencils text*/
		$stencils_text=implode("###$$$###",preg_replace('/œ/','ce',$corrector_params['stencil_text']));
		$stencils_text=utf8_decode($stencils_text);
	...
	public function checkStencilPlagarismAction()
	{
		...
		if(is_array($stencilsArray) && count($stencilsArray)>0)
		{
			...
			$articleDetails=$ebookerObj->getArticleDetails($article_id);
			...
			{ 
				...
				if($articleDetails)
				{
					$ebooker_cat_id=$articleDetails[0]['ebooker_cat_id'];
					$language=$articleDetails[0]['language'];
				}	

				//check Database plagarism
				$dbPlagResultArray=$this->chekcDBStencilsTextPercentage($article_id,$articleTokens,$stencilsArray,$ebooker_cat_id,$language);
	...
	function chekcDBStencilsTextPercentage($article_id,$tokens,$stencilsArray,$ebooker_cat_id=NULL,$language=NULL)
	{
		$ebookerObj=new Ep_Article_Ebooker();
		/*getting all DB stencils except given articles*/
		
		$DBStencils=$ebookerObj->getAllDBStencils($article_id,$ebooker_cat_id,$language);
		if(!$DBStencils)
			$DBStencils=array();		
		...
		if($language!='uk')
			$sampleTexts=$ebookerObj->getAllSampleText($ebooker_cat_id);
		
		if(!$sampleTexts)
			$sampleTexts=array();		
		//echo "<pre>";print_r($sampleTexts);exit;
		
		$allDBStencils=array_merge($DBStencils,$sampleTexts);
